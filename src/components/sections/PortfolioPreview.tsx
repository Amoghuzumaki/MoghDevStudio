...
const projects = [
  ...
  },
  {
    ...
    gradient: "from-slate-900/80 via-zinc-800/40 to-transparent",
    accent: "bg-red-500",
    emoji: "💪",
  ...
  },
  {
    ...
    gradient: "from-rose-900/80 via-pink-800/40 to-transparent",
    accent: "bg-rose-500",
    emoji: "👗",
          ...
          {projects.map((project, i) => (
            <AnimateOnScroll key={project.title} delay={i * 120}>
              <div className="group relative overflow-hidden rounded-2xl border border-white/5 hover:border-primary-600/40 transition-all duration-300 hover:shadow-2xl hover:shadow-primary-600/10 card-hover">
                {/* Mock browser window */}
                <div className={`relative h-52 ${project.mockColor} flex items-center justify-center overflow-hidden`}>
                  {/* Browser bar */}
                  <div className="absolute top-0 left-0 right-0 h-8 bg-slate-900/60 backdrop-blur flex items-center px-3 gap-1.5 border-b border-white/5">
                    ...
                    <div className="flex-1 mx-3 h-4 bg-white/5 rounded flex items-center px-2">
                      <span className="text-slate-500 text-[9px]">moghdevstudio.com/{project.title.toLowerCase().replace(/\s/g, '-')}</span>
                    </div>
                  </div>
                ...
                <div className="p-5 bg-slate-900/60">
                  ...
                  <div className="flex flex-wrap gap-2">
                    {project.tags.map((tag) => (
                      <span key={tag} className="px-2.5 py-1 bg-white/5 rounded-full text-xs text-slate-400">
                        {tag}
                      </span>
                    ))}
                  </div>
